...
\begin{algorithm}[h]
  \input{./input/algorpcbroadcastsplitA.tex}
  \caption{\label{algo:rpcbroadcastsplitA}Safety check of \RPCBROADCAST at Process $p$.}
\end{algorithm}

\begin{algorithm}[h]
  \input{./input/algorpcbroadcastsplitB.tex}
  \caption{\label{algo:rpcbroadcastsplitB}Dissemination and buffering of
    \RPCBROADCAST at Process $p$.}
\end{algorithm}
...
\begin{definition}[\label{def:extension}Safe link memory]
  A link from Process~A to Process~B must remember among Process~B's delivered
  messages those that will be received from Process~A and must forget those that
  will never: $\forall m,\, remember_{BA}(m)\equiv d_B(m) \rightarrow r_{BA}(m)$
\end{definition}
...
%   The buffer $\mathcal{B}$ includes messages delivered by Process~A but not
%   delivered by Process~B before the receipt of $\alpha$. This excludes messages
%   delivered by Process~A before the sending of $\alpha$ and messages delivered
%   by Process~B before the receipt of $\alpha$.
