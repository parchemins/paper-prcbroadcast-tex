...
\subsection{Link-wise memory}

We define link-wise memory as a mean for processes to forbid multiple delivery
...
\begin{definition}[\label{def:memory}Link-wise memory]
...
\begin{proof}
  ...
  a link carries a message at most once, it cannot receive another $m$ from
...
\subsection{Initializing link memory}
...
$O(Q_i \cdot M)$ where $M$ is the number of messages already delivered that should be
...
advantageous trade-off that depends on the actual system instead of past
