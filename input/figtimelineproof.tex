...
\begin{tikzpicture}[scale=1]
  ...
  \newcommand\X{1.8*\columnwidth/16pt};
  \newcommand\YA{0pt};
  \newcommand\YB{-60pt};
  ...
  \draw[<->] (3*\X, \YB - 1*\YSPACE) --
  node[below]{$\bm{B_\alpha} = \mathcal{D}\setminus\mathcal{B} = \mathcal{B}' \cup \mathcal{C}'$}
  (5*\X, \YB - 1*\YSPACE);
  ...
  \draw[<->] (4*\X, \YA +2*\YSPACE) -- node[above, yshift=-0.55ex]{$\bm{B_\beta} = \mathcal{E}\setminus\mathcal{C}= \mathcal{C}'\cup \mathcal{D}'$} (6*\X, \YA + 2*\YSPACE);


  \draw[->, very thick] ( 6*\X, \YA ) -- node[sloped, above]{$\bm{B_\beta}$} (7*\X, \YB);

  
  \draw[<->] (5*\X, \YB - 2*\YSPACE ) -- node[below, align=center]{$\bm{B_\pi}= \mathcal{G} \cup \mathcal{H}$\\with $\mathcal{G} \subseteq \mathcal{D}'$ and $\mathcal{H} \cap \mathcal{D}' = \varnothing$} 
  (7*\X, \YB - 2*\YSPACE);
