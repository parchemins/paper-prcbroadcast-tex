...
\begin{abstract}
  ...
  implementations rely on specific dissemination topologies, or on monotonically
  increasing data structures. They do not scale in large and dynamic systems.
  In this paper, we propose an original implementation of causal
  broadcast. Message overhead is constant. Local space complexity is
  non-monotonic and depends on system settings and its current usage. We prove
  that all and only obsolete control information are discarded, at the cost of
  few lightweight control messages.  Our implementation constitutes a better and
