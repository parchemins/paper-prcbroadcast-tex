...
% save delivered messages to ignore additional receipts. 
% In static systems, such data structure can be
% non-monotonic~\cite{raynal2013distributed}. We define a non-monotonic link-wise
% memory to forbid multiple delivery.
...
\begin{figure*}
  \begin{center}
    \subfloat[\label{fig:memorylinkA}Process~B broadcasts $b$ and awaits a copy
    of $b$ from Process~A.]
    {\input{input/figmemorylinkA.tex}}
    \hspace{30pt}
    \subfloat[\label{fig:memorylinkB}Process~A receives, delivers, and forwards
    $b$. It expects a copy from Process~C.]
    {\input{input/figmemorylinkB.tex}}
    \hspace{30pt}
    \subfloat[\label{fig:memorylinkC}Process~B and Process~C receive $b$. They
    do not expect additional copies. Process~C delivers and forwards $b$.]
    {\input{input/figmemorylinkC.tex}}
    \hspace{30pt}
    \subfloat[\label{fig:memorylinkD}Process~A receives its last expected copy
...
\begin{figure*}
  \begin{center}
    \subfloat[\label{fig:memorylinkfailsA}Process~A broadcasts $a$ and expects a copy
    from both Process~B and Process~C.]
    {\input{input/figmemorylinkfailsA.tex}}
    \hspace{30pt}
    \subfloat[\label{fig:memorylinkfailsB}Process C receives, delivers, and
    forwards $a$. It does not expect additional copies.]
    {\input{input/figmemorylinkfailsB.tex}}
    \hspace{30pt}
    \subfloat[\label{fig:memorylinkfailsC}Process~B adds a link to Process~C. 
    Then it receives, delivers, and forwards $a$.]
    {\input{input/figmemorylinkfailsC.tex}}
    \hspace{30pt}
    \subfloat[\label{fig:memorylinkfailsD}Process~C receives and mistakes $a$
...
\TODO{This comes from the fact that everyone broadcasts and forwards so we can
  tag links accordingly.}
...
%   \begin{itemize}
%   \item First delivery:
%     \small{$\forall m,\, remember_{BA}(m) \equiv d_B(m) \wedge \neg r_{BA}(m)$}
%   \item Eventual receipt: $\forall m,\, remember_{BA}(m) \implies r_{BA}(m)$
%   \end{itemize}
% \end{definition}
...
\begin{proof}
  ...
  % To deliver $m$ after its receipt from Process~A, Process~B must not remember
  % $m$ on this link.  If Process~B does not remember $m$ from Process~A, either
  % it already received $m$ from Process~A; or it did not received $m$ from any
  % process. In the former case, it cannot receive, hence deliver $m$ from
  % Process~A; in the latter case, receiving $m$ from Process~A leads to the
  % delivery of $m$. Again, since a link carries a message at most once, it cannot
  % receive another $m$ from Process~A that would lead to another delivery.
\end{proof}
...
%Figure~\ref{fig:memorylink} shows that 
...
\begin{figure*}
  \begin{center}
    \subfloat[\label{fig:solveA}Process~B adds a link to
    ...
    \hspace{30pt}
    \subfloat[\label{fig:solveB}Process~A receives $\alpha$ and routes it to
    Process~C.]
    {\input{input/figsolveB.tex}}
    \hspace{30pt}
    \subfloat[\label{fig:solveC}Process~C receives $\alpha$ and answers by
    ...
    \\
    \subfloat[\label{fig:solveD}Process~A receives $\beta$ and routes it to
    Process~B.  Process~A receives $c_1$ and forwards it to both its
    neighbors.]
    {\input{input/figsolveD.tex}}
    \hspace{30pt}
    \subfloat[\label{fig:solveE}Process~C receives and discards
    ...
    \\
    \subfloat[\label{fig:solveF}Process~A receives $c_1$ and discards it.
    ...
    \hspace{30pt}
    \subfloat[\label{fig:solveG}Process~A receives $c_2$ and forwards it to
    ...
    \\
    \subfloat[\label{fig:solveH}Process~A receives and discards $b_1$.
    ...
    \hspace{30pt}
    \subfloat[\label{fig:solveI}Once Process~A sent $B_\beta$, the new link is
    ...
    \\
    \subfloat[\label{fig:solveJ}Process~C categorizes each message of $B_\beta$
...
information associated to each link in the in-view. \TODO{$Q_i$ is generally far
  smaller that $P$, the number of processes in the system +\REF.}
