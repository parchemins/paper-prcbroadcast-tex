...
In Figure~\ref{fig:counterproblem}, the issue comes from Process~C being unable
to remember messages it will receive from Process~B's buffer (i.e., the message
$a$) while forgetting about messages it will never receive from Process~B (i.e.,
the message $b$).
...
\begin{figure*}
  \begin{center}
    ...
    \caption{\label{fig:timelineproof}\label{fig:timeline}Timeline of
      ...
      $B_beta$ using the new link. Messages arrive in causal order.
