...
Figure~\ref{fig:timelineproof} depicts the principle of \RPCBROADCAST. When a
...
Process~B can start gathering control information about its delivered messages
in a buffer $B_\alpha$. Among other, Process~B wants to identify messages
concurrent to the correct establishment of the new link. Process~B acknowledges
Process~A's notification using a control message $\beta$. When Process~A
receives $\beta$, it removes obsolete information: messages delivered by
Process~B before the sending of the acknowledgment
...
% \RPCBROADCAST implements safe link memory in dynamic systems.
