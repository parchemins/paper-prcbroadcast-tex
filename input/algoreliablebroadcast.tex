...
\Initially {} {
  ...
  $D \leftarrow \varnothing$ \tcp*{link $\in Q_i$ $\rightarrow$  messages to discard}
}
...
\Dissemination{}{
  ...
  \Function{\textup{received}($m,\, l$)}{
    ...
    \If {$\not\exists q \in D$ \textbf{\textup{such that}} $m \in D[q]$} {
      \lForEach {$q \in Q_i$} {$D[q] \leftarrow D[q] \cup m$}
      $hasReceived \leftarrow false$ \;
    }
    $D[l] \leftarrow D[l] \setminus m$ \;
    \Return $hasReceived$ \;
  }
