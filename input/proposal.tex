...
\begin{proof}
  ...
  Process~A that would lead to another delivery. \TODO{Missing non-monotonic.}
...
\PCBROADCAST (\REF) constitutes an efficient and lightweight implementation of
...
already implements the maintenance of safe link memory over receipts. Processes
discards obsolete control information over receipts.  However, new safe links
require consistent initialization. The challenge consists in initializing such
...
Figure~\ref{fig:timelineproof} depicts the functioning of \RPCBROADCAST. When a
Process~A adds a link to Process~B, \RPCBROADCAST initializes safe link memory
in 3 steps.

Process~A notifies Process~B using safe links. At receipt, Process~B implicitly
removes obsolete information: messages delivered by Process~A before the sending
of the notification $\mathcal{A}_1$. At receipt, Process~B can start gathering
control information about its delivered messages in a buffer $B_\alpha$. Among
other, Process~B wants to identify messages concurrent to the safety
check. Process~B acknowledges Process~A's notification. When the latter receives
the acknowledgment, it starts the safety check of the new link (\REF). Once
again, it removes obsolete information: messages delivered by Process~B before
the sending of the acknowledgment $\mathcal{A}_1 \cup \mathcal{B}_1$. Process~A
sends a control message $\pi$ to Process~B using safe links, and starts to
gather control information about its delivered messages in a buffer $B_\beta$ in
order to ensure safe links. Upon receipt, Process~B closes its buffer
$B_\alpha$.
...
\begin{lemma}[\label{lem:balpha}Messages in buffer $B_\alpha$]
  ...
  sending of its acknowledgment $\beta$ and before the receipt of the control
  message $\pi$.\\
...
\begin{proof}
  Since safe links provides causal order among messages they convey, all
  ...
  ($\mathcal{B}_2$). \TODO{Double check.}
\end{proof}

Upon receipt of $\pi$, Process~B continues to gather control information about
...
yet. It sends the last acknowledgment $\rho$ to Process~A using safe
links. Upon receipt of this acknowledgment, Process~A closes its buffer and
sends it using the new link. Afterwards the link is safe to use by Process~A for
causal broadcast (see Definition~\ref{def:safe}).
...
\begin{proof}
  ...
  difference being that an unsafe link conveys $B_\beta$. Hence, Process~B
  cannot state if it received all, part, or none of messages in
  $\mathcal{A}_3$. Thus, $\mathcal{A}_3' \subseteq \mathcal{A}_3$.
\end{proof}

Using $B_\alpha$, $B_\beta$, and $B_\pi$ buffers, Process~B identifies messages
in $B_\beta$ it must deliver against messages it must ignore, and messages in
$B_\pi$ it must receive from Process~A. This allows Process~B to initialize safe
link memory.

\begin{theorem}[$B_\alpha$, $B_\beta$, and $B_\pi$ build safe links and their
  memory]
  A new link becomes safe at the sending of $B_\beta$. The memory of this new
  safe link becomes correct at receipt of $B_\beta$.
\end{theorem}

\begin{proof}
  The proof that a new link becomes safe at sending of $B_\beta$ is similar to
  that of \PCBROADCAST (\REF).  We must show that the memory of a new safe link
  becomes correct at receipt of $B_\beta$. \\ 
...
\TODO{Change prior things so it does not use safe links to travel but require
  causal order.}  Algorithm~\ref{algo:rpcbroadcast} shows the instructions of
\RPCBROADCAST. It implements safe link memory in dynamic systems.
