...
\begin{definition}[Safe link extension]
  ...
  % can differentiate messages it delivered from messages it
  %  did not delivered when received via this link.
  %  that will be received via this new link
  
  % does not deliver the messages received via this new link if it already
  % delivered this message, and delivers it otherwise.
\end{definition}
...
\begin{definition}[Expect \TODO{not here}]
...
\begin{definition}[Deliver \TODO{not here}]
...
The proposed broadcast interlaces 3 buffering phases. A control message $beta$
...
% \begin{definition}[meow \TODO{todo}]
%   The buffer includes the following messages:

%   Since control messages travel by reliable FIFO links, the buffer includes the
%   following messages from Process~A:
%   \begin{equation*}
%     \forall m,\, s_A(\pi_1) \rightarrow d_A(m) \wedge d_A(m) \rightarrow s_A(\pi_2) \implies m \in B
%   \end{equation*}
%   The buffer excludes all other messages. It excludes messages delivered before
%   and after the receipt of control messages:
%   \begin{equation*}
%   \end{equation*}
%   It excludes messages delivered by Process~A:
%   \begin{equation*}
%     \forall m,\, d_A(m) \rightarrow s_A(\pi_1) \implies m\not\in B
%   \end{equation*}
% \end{definition}
