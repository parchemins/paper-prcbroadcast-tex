...
\begin{definition}[Exactly once delivery]
  ...
  receive a broadcast message multiple times but delivers it exactly once. \\
%  $\forall m, \{r_A(m)\}^+ \rightarrow d_A(m) \wedge d_A(m) \not\rightarrow d_A(m) \wedge d_A(m) \rightarrow \{r_A(m)\}^*$ \\
  $\forall A,\,B,\, b_A(m) \implies r_B(m)$ \\
  $\forall m,\, d_A(m) \implies \exists r_A(m),\, r_A(m) \rightarrow d_A(m)$ \\
...
\begin{theorem}[Remembering ensures exactly once delivery]
  \TODO{Assuming exactly once delivery?} Delivering broadcast messages when they
...
\begin{figure*}
  \begin{center}
    ...
    and registers $c_2$. Then Process~B receives $rho$ and sends $B_\beta$ to Process~C
    using the new link.]
    {\input{input/figsolveH.tex}}
    \hspace{10pt}
    \subfloat[Part I][Process~A receives and discards $b_1$. 
    Process~A receives and routes $\rho$ to Process~B. 
    Process~A receives and  forwards $b_2$ then $c_3$. Process~B receives, forwards, 
    and registers $c_2$. Then Process~B receives $rho$ and sends $B_\beta$ to Process~C
    using the new link.]
