...
When a Process~A adds a link to Process~B, \RPCBROADCAST initializes safe link
memory in 3 steps:
\begin{enumerate}
\item Process~A notifies Process~B using safe links. At receipt, Process~B
  implicitly removes obsolete information: messages delivered by Process~A
  before the sending of the notification $\mathcal{A}_1$. At receipt, Process~B
  can start gathering control information about its delivered messages in a
  buffer $B_\alpha$. Among other, Process~B wants to identify messages
  concurrent to the safety check. Process~B acknowledges Process~A's
  notification. When the latter receives the acknowledgment, it starts the
  safety check of the new link (\REF). Once again, it removes obsolete
  information: messages delivered by Process~B before the sending of the
  acknowledgment $\mathcal{A}_1 \cup \mathcal{B}_1$. Process~A sends a control
  message $\pi$ to Process~B using safe links, and starts to gather control
  information about its delivered messages in a buffer $B_\beta$ in order to
  ensure safe links. Upon receipt, Process~B closes its buffer $B_\alpha$.
  ...
  \begin{lemma}[\label{lem:balpha}Messages in buffer $B_\alpha$]
    ...
    message $\pi$.\\
    This includes messages delivered by Process~A before the sending of $\pi$
    that were not delivered by Process~B before the sending of $\beta$:
    $\mathcal{A}_2$. Most importantly, this also includes messages delivered by
    Process~B that were not delivered by Process~A at the sending of $\pi$:
    $\mathcal{B}_2$.
  \end{lemma}

  \begin{proof}
    Since safe links provides causal order among messages they convey, we have:
...
\item Upon receipt of $\pi$, Process~B continues to gather control information
  about its delivered messages in another buffer $B_\pi$. Some messages in this
  buffer will be expected from Process~A, but Process~B cannot determine which
  ones just yet. It sends the last acknowledgement $\rho$ to Process~A using
  safe links. Upon receipt of this acknowledgment, Process~A closes its buffer
  and sends it using the new link. Afterwards the link is safe to use by
  Process~A for causal broadcast (see Definition~\ref{def:safe}).
  ...
  \begin{lemma}[\label{lem:bbeta}Messages in buffer $B_\beta$]
    ...
    This includes messages delivered by Process~B before the sending of $\rho$
    that were not delivered by Process~A before the sending of $\pi$:
    $\mathcal{B}_2$. This also includes messages delivered by Process~A that
...
\item Upon receipt of $B_\beta$, Process~B is able to identify messages in
  $B_\beta$ it must deliver against messages it must ignore, and messages in
  $B_\pi$ it must receive from Process~A.  \TODO{To continue.}
\end{enumerate}
