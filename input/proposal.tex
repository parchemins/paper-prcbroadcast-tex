...
Algorithms~\ref{algo:counterrbroadcast} and~\ref{algo:reliablebroadcast}
...
broadcasting a message sends the message to an out-view of safe links that
cannot lead to causal order violations.
...
sending control messages. Figure~\ref{fig:timelinepcbroadcast} shows the
operation. Process~A sends a control message $\pi$ to Process~B using safe links
and awaits its answer $\rho$. This is a ping phase.
...
However, this is insufficient to ensure exactly once delivery (\TODO{With
  bounded remembering}). The issue is identical to that depicted in
Figure~\ref{fig:generalproblem}. Process~B adds Process~C in its out-view but
Process~C already forgot about the message $a$ while Process~B received it. This
leads to double delivery of $a$ and subsequent inconsistencies.
...
\TODO{Must be self contained. Must be related to issues and motivations...}
...
\begin{figure*}
  \begin{center}
    ...
    \caption{\label{fig:timelineproof}\label{fig:timeline}Timeline of
      ...
      hide intermediate processes for the purpose of clarity. Messages arrive in
      causal order.
...
phase. Figure~\ref{fig:timeline} depicts the functioning of
\RPCBROADCAST. Process~A adds a link to Process~B. \RPCBROADCAST ensures link
safety in few steps:

\begin{enumerate}[leftmargin=*]
\item Process~A starts a first buffering phase. It sends a first control message
  $\alpha$ to Process~B using safe links (see Line~\ref{line:sendalpha}).
\item After being routed to Process~B by intermediary processes, $\alpha$
  reaches Process~B.  Process~B starts to register messages it delivers in a
  buffer $B_\alpha$.  Process~B starts a second buffering phase. It sends a
  second control message $\beta$ to Process~A using safe links (see
  Line~\ref{line:sendbeta}).
\item After being routed to Process~A, $\beta$ reaches Process~A. Process~A
  starts to register messages it delivers in a buffer $B_\beta$.  Process~A
  starts a third buffering phase. It sends a third control message $\pi$ to
  Process~B using safe link (see Line~\ref{line:sendpi}).
\item After being routed to Process~B by intermediary processes, $\pi$ reaches
  Process~B. Process~B ends the first buffering phase: Process~B stops buffering
  messages in $B_\alpha$.  Process~B starts to register messages it delivers in
  $B_\pi$.  Process~B sends a fourth and last control message $\rho$ to
  Process~A using safe links (see Line~\ref{line:sendrho}).
\item After being routed to Process~A, $\rho$ reaches Process~A.  Process~A
  stops buffering and sends its buffer of messages $B_\beta$ using the new link
  $s_{AB}(B)$ (see Line~\ref{line:sendbuffer}). The new link is safe. Process~A
  starts to use this link normally for causal broadcast.
\item Once Process~B receives the buffer, it ends the third buffering
  phase. Using $B_\alpha$, and $B_\pi$, Process~B is able to distinguish among
  messages from $B_\beta$: the array of messages to deliver (see
  Line~\ref{line:todeliver}) from the set of messages to ignore, i.e., the rest
  of messages. Process~B also identifies among its delivered messages the
  messages to expect from Process~A (see Line~\ref{line:toexpect}). Messages
...
\begin{figure*}
  \begin{center}
    ...
    Process~B receives $c_1$ and forwards it to its neighbor.
    Process~B receives $\beta$ and replies $\pi$ to Process~C using Process~A 
...
policy. Such strategy fits \RPCBROADCAST. Both involved processes can halt and
retry the safety check of the new link. Either the origin process does not send
its buffer $B_\beta$, or the destination process does not accept incoming
messages.

% \TODO{Receiving may ignore all incoming messages.}
...
be discarded or delivered. \TODO{Maybe $O(Q_i)$?}
