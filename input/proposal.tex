...
\begin{figure}
  \begin{center}
    ...
    \begin{subfigure}[t]{0.23\textwidth}
      ...
      \caption{\label{fig:memorylinkC}Process~B and Process~C receive $b$. They do
not expect additional copies. Process~C delivers and forwards $b$.}
    \end{subfigure}
    ~
    \begin{subfigure}[t]{0.23\textwidth}
    ...
    \end{subfigure}
    \caption{Link memory allows processes to forget past deliveries in static systems.}
  \end{center}
\end{figure}
...
process can receive an already forgotten message from any other process. This
may lead to double delivery with cascading effects in the
...
solve this issue by exploiting causal broadcast's ability to ensure a specific
order on message delivery. To characterize the order among events such as send,
or receive, we define time in a logical sense using Lamport’s definition.
...
\PCBROADCAST (\REF) is a causal broadcast protocol that provides causal order at
...
\begin{lemma}[Messages of buffer $B_\alpha$]
\end{lemma}


\begin{theorem}[\RPCBROADCAST solves the problem statement]
  \TODO{Meow}
\end{theorem}
